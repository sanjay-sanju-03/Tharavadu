  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      if (searchType === 'all' || searchType === 'members') {
        combinedResults = membersResponse.data.map(member => ({
          type: 'member',
          ...member
        }));
      }

      if ((searchType === 'all' || searchType === 'payments') && membersResponse.data.length > 0) {
        const paymentsResponse = await axios.get('http://localhost:5000/api/payments', {
          params: { search: searchQuery }
        });
        const paymentResults = paymentsResponse.data.map(payment => ({
          type: 'payment',
          ...payment
        }));
      ...
      <div className="search-bar" style={{
        ...
        <form onSubmit={handleSearch}>
          <div style={{ flex: 1, minWidth: '300px', position: 'relative' }}>
            <input
              type="text"
              placeholder="Type to search by Name, ID, Phone, or Email..."
              value={searchQuery}
          ...
          {results.length > 0 && (
            <div style={{
              ...
              <div style={{
                ...
                <div>
                  <div style={{ fontSize: '12px', color: 'var(--neutral-500)', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                    Total Results
                  </div>
                  <div style={{ fontSize: '28px', fontWeight: '800', color: 'var(--neutral-800)', letterSpacing: '-1px' }}>
                    {results.length}
                  </div>
                </div>
              </div>
              {memberResults.length > 0 && (
                <div style={{
                  ...
                  <div>
                    <div style={{ fontSize: '12px', color: 'var(--primary-600)', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                      Members
                    </div>
                    <div style={{ fontSize: '28px', fontWeight: '800', color: 'var(--primary-700)', letterSpacing: '-1px' }}>
              ...
              {paymentResults.length > 0 && (
                <div style={{
                  ...
                  <div>
                    <div style={{ fontSize: '12px', color: '#059669', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                      Payments
                    </div>
                    <div style={{ fontSize: '28px', fontWeight: '800', color: '#047857', letterSpacing: '-1px' }}>
                  ...
                  <tbody>
                    {paymentResults.map(payment => (
                      <tr key={payment.id}>
                        ...
                        <td style={{ fontWeight: '700', color: 'var(--neutral-800)', fontSize: '16px' }}>
        ...
        <div className="card">
          <div style={{
            ...
            <div style={{
              ...
              {['Ravi', 'T001', '9876'].map((suggestion, index) => (
                <button
                  key={suggestion}
                  type="button"
                  onClick={() => {
